...
- Mass Drivers are gameplay: a building that has to be fed over the conduit network like every other consumer, a standing order per world (cargo and destination) set on the solar map, pods that take real transit time measured off the two worlds' orbits, and cargo that lands in the target world's stockpile. A world left behind keeps throwing at the world the swarm moved to, and the map draws the pods crossing between the orbits. Nothing teleports: a driver with no route is not a destination, and a driver nothing is routed to throws nothing.
- The receiving end is still a pool rather than a place: a pod lands in the target world's stockpile, not on a landing pad that has to be built, powered and routed from. Until it does, the destination world's layout has no say in what it can accept.
...
- Grow the building set beyond 14 across processing, logistics, and megastructure parts. Hazard counters and the Mass Driver exist now; nothing else on that list does.
