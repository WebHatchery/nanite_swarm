...
- Ground carries an ore richness rolled at generation (`ore` in `game_config.json`): a drill cuts at its tile's rate, so where one goes is a spatial decision. Only the bonus above ordinary ground depletes, and only for ore actually cut (rationale in `gdd.md` §3). The inspector reads the ground out. What the *map* does not do is show it — a deposit is invisible until you point at it, so prospecting means sweeping the cursor over the whole world.
